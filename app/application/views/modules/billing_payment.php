...
if ( $PaymentOption == "PayPal")
{
        ...
        //' Shipping page.
        //'------------------------------------
        $userInfo = json_decode($this->session->userdata('comments'));
        $shipToName = $this->session->userdata('comments');
        $shipToStreet = $userInfo->address1;
        $shipToStreet2 = $userInfo->address2; //Leave it blank if there is no value
        $shipToCity = $userInfo->city;
        $shipToState = $userInfo->state;
        $shipToCountryCode = $userInfo->country; // Please refer to the PayPal country codes in the API documentation
        $shipToZip = $userInfo->postal;
        ...
        //'------------------------------------
        $returnURL = base_url()."billing/review";
        ...
        //' This is set to the value entered on the Integration Assistant
        //'------------------------------------
        $cancelURL = base_url()."billing/cancelorder";
        ...
        if($ack=="SUCCESS" || $ack=="SUCCESSWITHWARNING")
        {
                ...
                $this->session->set_userdata('reshash', $token);
