...
class Simplelogin
{
    ...
    function create($user = '', $password = '') {
        ...
        if($user == '' OR $password == '') {
        	appendToPageLevelMessage('Username and password are required', 'error');
            return false;
        }
        ...
        if ($query->num_rows() > 0) {
        	...
        	appendToPageLevelMessage('The selected username already exists in the system', 'error');
            return false;
            ...
            $data = array(
            	...
            	'contact_name' => $this->CI->input->post('contact_name'),
            ...
            if(!$this->CI->db->insert($this->user_table)) {
                //There was a problem!
        		appendToPageLevelMessage('Unable to add user to database', 'error');
                return false;                        
            }
            $user_id = $this->CI->db->insert_id();
            
            //Login was successful            
			appendToPageLevelMessage('User successfully added');
            return true;
        }
